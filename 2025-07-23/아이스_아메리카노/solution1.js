...
function count(money) {
    return Math.floor(money / americano);
}

function change(money) {
    return money % americano;
}
...
function solution(money) {
    ...
    const americanoCount = count(money);
    const payChange = change(money);

    answer.push(americanoCount, payChange);
